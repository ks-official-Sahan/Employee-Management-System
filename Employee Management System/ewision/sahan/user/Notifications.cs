...
namespace Employee_Management_System.user_dashboard_pages
{
    public partial class Notifications : Form
    {
        ...
        private void LoadNotifications()
        {
            ...
            string query = "SELECT * FROM `notification` INNER JOIN `target` ON `target`.`id` = `notification`.`target_id` WHERE `user_type_id`='2'";
            ...
            MySqlDataReader resultSet = MySQL.Execute(query);
                ...
                while (resultSet.Read())
                {
                    ...
                    newRow.Cells[2].Value = string.Format("{0:yyyy/MM/dd}", resultSet["date_time"]);
